...
title: HTTP 缓存
date: 2026-04-25
description: "MDN HTTP 指南：HTTP 缓存"
categories:
  - 网络安全
  - HTTP
tags:
...
!!! info "来源声明"
    本文照搬整理自 MDN Web Docs 中文文档：[https://mdn.org.cn/zh-CN/docs/Web/HTTP/Guides/Caching](https://mdn.org.cn/zh-CN/docs/Web/HTTP/Guides/Caching)。
...
另一方面，如果 TLS 桥接代理通过在 PC 上安装来自组织管理的 CA 证书，以中间人方式解密所有通信，并执行访问控制等，则可以查看响应的内容并将其缓存。但是，由于[证书透明度（certificate transparency）](https://mdn.org.cn/zh-CN/docs/Web/Security/Defenses/Certificate_Transparency)在最近几年变得很普遍，并且一些浏览器只允许使用证书签署时间戳（signed certificate timestamp）颁发的证书，因此这种方法需要应用于企业策略。在这样的受控环境中，无需担心代理缓存“已过时且未更新”。
...
有关更多信息，请参阅 CDN 文档，并查阅 [service worker 文档](https://mdn.org.cn/zh-CN/docs/Web/API/Service_Worker_API)。
